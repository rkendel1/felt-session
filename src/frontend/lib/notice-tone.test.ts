...
describe("noticeTone", () => {
	...
	test("recovered-from trouble reads as a warning", () => {
		expect(noticeTone("Sandbox unavailable (egress blocked) — running on the host this turn.")).toBe(
			"warn",
		);
		expect(
	...
	test("routine operational chatter stays neutral", () => {
		expect(noticeTone("Session was busy — message queued; it sends when the current run finishes.")).toBe(
			"info",
		);
		expect(noticeTone("Holding 2 queued messages until the agent fully completes.")).toBe("info");
		expect(noticeTone("Opus 4.8 hit its usage limit — using Sonnet 5 for this turn only.")).toBe("info");
		expect(noticeTone("Still mid-task — auto-continuing first; 1 queued message.")).toBe("info");
	...
	test("a run that merely mentions failure is not itself a failure", () => {
		...
		expect(noticeTone("Background task update: the build failed, retrying")).toBe("info");
	});
});
...
test("stripNoticeGlyph drops a leading glyph the tone re-supplies", () => {
	...
	expect(stripNoticeGlyph("Turn stopped after 3 minutes — it hit ⚠ the limit")).toBe(
		"Turn stopped after 3 minutes — it hit ⚠ the limit",
	);
});
