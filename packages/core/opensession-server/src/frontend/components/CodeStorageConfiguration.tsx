...
export function CodeStorageConfiguration({
	...
	useEffect(() => {
		...
		const timer = window.setInterval(() => {
			void fetchCodeStorageStatus().then(setStatus).catch(() => {});
		}, 30_000);
	...
	async function connect() {
		...
		await (async () => {
			...
			setError(cause instanceof Error ? cause.message : "Could not connect code.storage");
	...
	async function disconnect() {
		...
		try {
			...
			setError(
				cause instanceof Error ? cause.message : "Could not disconnect code.storage",
			);
		}
	}
		...
		<SettingsSection className={mergeStylexOverrideClassName("", sx.p4)}>
			<div {...stylex.props(sx.flex, sx.flexWrap, sx.itemsStart, sx.gap3)}>
				<div {...stylex.props(sx.minW12rem, sx.flex1)}>
					<div {...stylex.props(sx.fontMedium, sx.textFg, typography.itemTitle)}>Connection</div>
					<div {...stylex.props(sx.mt05, sx.leadingSnug, sx.textDim, typography.supporting)}>
						{!connected
				...
				{connected && (
					<Button
						size="sm"
						className={mergeStylexOverrideClassName("hover:border-red hover:text-red phone:min-h-11", sx.shrink0)}
			...
			{error && (
				<InlineAlert className={mergeStylexOverrideClassName("", sx.mt4)} onDismiss={() => setError(null)}>
					{error}
				</InlineAlert>
			)}
			{note && <div {...stylex.props(sx.mt3, sx.textDim, typography.supporting)}>{note}</div>}
			...
			{!connected ? (
				<div {...stylex.props(sx.mt4, sx.flex, sx.flexCol, sx.gap4, sx.borderT, sx.borderLine, sx.pt4)}>
					...
					<SettingsField>
						...
						/>
						<span {...stylex.props(sx.fontNormal, sx.textFaint, typography.supporting)}>
				...
				<>
					{status.error && <div {...stylex.props(sx.mt3, sx.textRed, typography.supporting)}>{status.error}</div>}
					{webhook && (
						<div {...stylex.props(sx.mt4, sx.flex, sx.flexCol, sx.gap2, sx.borderT, sx.borderLine, sx.pt4)}>
							...
							<div {...stylex.props(sx.flex, sx.flexWrap, sx.itemsCenter, sx.gap2, sx.textDim, typography.label)}>
								...
								<Button size="sm" variant="ghost" onClick={() => void copy(webhook.path, "path")}>
							...
							<div {...stylex.props(sx.flex, sx.flexWrap, sx.itemsCenter, sx.gap2, sx.textDim, typography.label)}>
								...
								<Button size="sm" variant="ghost" onClick={() => setShowSecret((shown) => !shown)}>
									{showSecret ? "Hide" : "Reveal"}
								</Button>
								<Button size="sm" variant="ghost" onClick={() => void copy(webhook.secret, "secret")}>
									{copied === "secret" ? "Copied" : "Copy"}
								</Button>
							</div>
							<div {...stylex.props(sx.leadingSnug, sx.textFaint, typography.supporting)}>
								Add the public URL and secret in Pierre → Webhooks. Subscribe to push and repo.sync events.
							</div>
							<div className={cn(utilityClassName("text-meta"), lastDelivery && !lastDelivery.ok ? utilityClassName("text-red") : "text-faint")}>
								{!lastDelivery
							...
							{webhook.lastRejected && (
								<div {...stylex.props(sx.leadingSnug, sx.textRed, typography.supporting)}>
									{webhook.rejectedCount} unauthenticated request{webhook.rejectedCount === 1 ? "" : "s"} rejected ({webhook.lastRejected.error}), last {relativeTime(webhook.lastRejected.at)}.
								</div>
							)}
							{webhook.syncFailures.map((failure) => (
								<div key={failure.repo} {...stylex.props(sx.textRed, typography.meta)}>
									Sync failing for {failure.repo}: {failure.error} ({relativeTime(failure.at)})
								</div>
							))}
						</div>
					)}
