...
function result(forId: string, content: string, isError = false): TranscriptEntry {
	return {
		id: `${forId}-r`,
		type: "tool_result",
		content,
...
describe("commandHead", () => {
	it("drops cd preambles and keeps program + first argument", () => {
		expect(commandHead("cd /home/ubuntu/projects/x && bun test src/server")).toBe("bun test");
...
describe("evidenceFromTranscript", () => {
	it("pairs an errored result with the call that produced it", () => {
		...
		expect(failures[0]).toMatchObject({ tool: "edit", what: "src/server/x.ts", laterPassed: false });
...
function evidenceDeps(over: Partial<EvidenceDeps> = {}): EvidenceDeps {
	return {
		...
		transcript: async () => [],
		diff: async () => ({
			files: [{ path: "src/a.ts", status: "modified", additions: 10, deletions: 2 }],
			totalAdditions: 10,
			totalDeletions: 2,
		}),
...
describe("collectHandoffEvidence", () => {
	it("gathers diff, PR, usage and failures for a finished worker", async () => {
		...
		expect(ev!.pr).toEqual({ url: "https://github.com/x/y/pull/1", state: "OPEN" });
	...
	it("reports the run error and still returns evidence when git fails", async () => {
		const ev = await collectHandoffEvidence(
			...
			evidenceDeps({
				getSession: () =>
					session({ lastRunError: { message: "Usage limit reached", at: "2026-07-24T10:00:00Z" } }),
				diff: async () => {
...
describe("formatHandoffEvidence", () => {
	const ev: HandoffEvidence = {
		...
		failures: [
			{ tool: "bash", what: "bun test", errorTail: "1 fail", laterPassed: true },
		],
...
describe("notifyParentOfFailedRun", () => {
		...
		const b = beacon(
			{ parentSessionId: "bks-parent", reportBack: true },
			{
				evidence: async () => ({
					sessionId: "bks-child",
					state: "error",
					failures: [{ tool: "bash", what: "bun test", errorTail: "1 fail", laterPassed: false }],
					commands: ["bun test"],
				}),
			},
		);
	...
	it("stays quiet for a session with no parent", async () => {
		...
		expect(await notifyParentOfFailedRun("bks-solo", "boom", b.deps)).toBe("no-parent");
	...
	it("stays quiet for a child told not to report back", async () => {
		...
		expect(await notifyParentOfFailedRun("bks-child", "boom", b.deps)).toBe("no-parent");
	});
	...
	it("defers to the worker's own report when it just sent one", async () => {
		...
		expect(await notifyParentOfFailedRun("bks-child", "boom", b.deps)).toBe("already-reported");
		expect(b.delivered).toHaveLength(0);
	});
	...
	it("throttles repeat beacons from a worker failing in a loop", async () => {
		...
		expect(await notifyParentOfFailedRun("bks-child", "boom", b.deps)).toBe("throttled");
		expect(b.delivered).toHaveLength(0);
	});
	...
	it("never throws out of a run-end path", async () => {
		...
		expect(await notifyParentOfFailedRun("bks-child", "boom", b.deps)).toBe("failed");
	});
});
