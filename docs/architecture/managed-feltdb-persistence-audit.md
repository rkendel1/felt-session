...
| `~/.opensession/config.json` | `config.ts` reads instance configuration per call; `config-mutation.ts` writes it. The current pairing route stores `integrations.feltdbRuntimeHandoffs` here. This makes managed workspace identity depend on a local JSON record. | Deployment configuration stays operator-owned, but managed authority identity comes from `feltdb.config.json` plus secret environment variables. Pairing state belongs in `opensession_pairings`; handoff entities stay in the shared canonical handoff collection. |
