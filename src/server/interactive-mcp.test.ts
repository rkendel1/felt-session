...
describe("interactive opensession-sessions MCP", () => {
	test("publishes and applies the createdBy filter through run-rpc", async () => {
		...
		try {
			...
			if (listed.kind !== "immediate") throw new Error("expected tools/list response");
			...
			if (called.kind !== "call") throw new Error("expected tools/call response");
