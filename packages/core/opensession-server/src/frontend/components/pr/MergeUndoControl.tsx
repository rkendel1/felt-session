...
export function MergeUndoControl({
  ...
  return (
    <div
      className={cn(
        mergeStylexClassName("", sx.inlineFlex, sx.shrink0, sx.itemsStretch, sx.whitespaceNowrap, sx.bgFg8, sx.textDim),
        compact
          ? mergeStylexClassName("", sx.minH22px, sx.roundedMd, typography.label)
          : mergeStylexClassName("", sx.minH26px, sx.roundedControl, sx.textXs, sx.phoneMinH26px),
        className,
      )}
    >
      <span
        aria-live="polite"
        className={cn(
          mergeStylexClassName("", sx.flex, sx.itemsCenter, sx.fontMedium, sx.TextBoxTrimBothCapAlphabetic),
          compact ? mergeStylexClassName("", sx.px2) : mergeStylexClassName("", sx.px25, sx.phonePx15),
        )}
      >
        PR merged
      </span>
      <Tooltip label="Undo" shortcut={UNDO_SHORTCUT_KEYS}>
        <Button
          ...
          onClick={onUndo}
          className={cn(
            mergeStylexClassName("", sx.relative, sx.roundedLNone, sx.beforeAbsolute, sx.beforeInsetY15, sx.beforeLeft0, sx.beforeWPx, sx.beforeBgLine),
            compact
              ? mergeStylexClassName("", sx.minH22px, sx.roundedRMd, sx.px2, typography.label)
