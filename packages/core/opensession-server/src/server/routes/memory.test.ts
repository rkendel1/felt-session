...
describe("memory v2 routes", () => {
	test("creates, pages, reads details, pins, and archives structured memory", async () => {
		const createdResponse = await handleMemoryRoutes(context("/api/memory", {
			method: "POST",
		...
		const pinned = await (await handleMemoryRoutes(context(
			`/api/memory/${created.entry.id}`,
			{
				method: "PATCH",
				headers: { "content-type": "application/json" },
		...
		const repo = scopes.scopes.find((item: any) => item.scope.key === "repo-opensession");
	...
	test("rejects verbose summaries and non-expiring status", async () => {
		for (const body of [
			...
			const response = await handleMemoryRoutes(context("/api/memory", {
				method: "POST",
				headers: { "content-type": "application/json" },
	...
	test("preserves provenance when Settings edits an imported or agent-verified memory", async () => {
		...
		const response = await handleMemoryRoutes(context(`/api/memory/${original.id}`, {
			method: "PATCH",
			headers: { "content-type": "application/json" },
			body: JSON.stringify({
				scopeKey: "repo-opensession",
	...
	test("keeps another teammate's private memory out of direct route reads", async () => {
		...
		const ctx = context(`/api/memory/${privateRecord.id}?scopeKey=${privateRecord.scopeKey}`);
	...
	test("keeps the legacy rollback route usable but requires archive before delete", async () => {
		...
		const activeDelete = await handleMemoryRoutes(context(
			`/api/memory/${id}?scopeKey=repo-opensession&confirm=true`,
			{ method: "DELETE" },
		));
		expect(activeDelete?.status).toBe(400);
		const archived = await handleMemoryRoutes(context(`/api/memory/${id}`, {
			method: "PATCH",
			headers: { "content-type": "application/json" },
			body: JSON.stringify({ scopeKey: "repo-opensession", action: "archive" }),
		}));
		expect(archived?.status).toBe(200);
		const removed = await handleMemoryRoutes(context(
			`/api/memory/${id}?scopeKey=repo-opensession&confirm=true`,
			{ method: "DELETE" },
		));
