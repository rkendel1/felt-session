...
export function PlainRouterConfiguration() {
	...
	async function save(patch: { prompt?: string; basicModel?: string }) {
		...
		setError(null);
		await (async () => {
			const response = await fetch(`${BASE_PATH}/api/connections/plain-router`, {
				method: "PUT",
				headers: { "Content-Type": "application/json" },
				body: JSON.stringify(patch),
			});
			const body = await response.json();
			if (!response.ok) return Promise.reject(new Error(body.error || `Failed: ${response.status}`));
			setConfig((current) => (current ? { ...current, ...body } : current));
			if ("prompt" in patch) setDraft(body.prompt);
			setSavedAt(Date.now());
		})().catch(async (cause) => {
			setError(cause instanceof Error ? cause.message : "Could not save the triage router");
		});
		setSaving(false);
	}
