...
function fakeStore(rows: Row[], full = new Map<string, TranscriptEntry>()): ExcerptStore {
	return {
...
describe("excerptTerms", () => {
	it("keeps code-ish tokens and drops noise", () => {
		expect(excerptTerms("why did getSessionDiff fail on src/server/x.ts?")).toEqual([
			"why",
			"did",
			"getsessiondiff",
			"fail",
...
describe("transcriptExcerpt", () => {
	...
	it("falls back to the tail with no query", async () => {
		const ex = await transcriptExcerpt("bks-1", { limit: 5 }, deps(fakeStore(rows)));
	...
	it("hydrates returned entries to full content (store rows are bounded)", async () => {
		...
		const full = new Map<string, TranscriptEntry>([
			["e1", { ...stripped[0]!, content: "the whole thing, unclipped", contentClamped: false }],
		]);
		const ex = await transcriptExcerpt("bks-1", { limit: 2 }, deps(fakeStore(stripped, full)));
		expect(ex.windows[0]!.entries[0]!.content).toBe("the whole thing, unclipped");
...
describe("formatExcerpt", () => {
	it("tags every line with its seq and surfaces tool inputs and errors", async () => {
		const rows = [
			...
			entry(3, { type: "tool_result", toolUseId: "t2", content: "boom", isError: true }),
		];
