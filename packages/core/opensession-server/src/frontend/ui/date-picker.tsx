...
function RangeCalendar({
	...
	React.useEffect(() => {
		if (!months.some((m) => isSameMonth(focused, m))) setAnchor(startOfMonth(focused));
	}, [months, focused]);
	...
	function pageMonth(delta: number) {
		...
		const landing = delta < 0 ? next : startOfMonth(addMonths(next, monthCount - 1));
	...
	const bandFrom = pending ? (preview && preview < pending ? preview : pending) : from;
	const bandTo = pending ? (preview && preview > pending ? preview : pending) : to;
										...
										{week.map((day, i) => {
											...
											return (
												<Day
													...
													span={
														span && {
															open: span.open || !isSameMonth(week[i - 1] ?? "", month),
															close: span.close || !isSameMonth(week[i + 1] ?? "", month),
														}
													}
													onPick={pick}
