		...
		<SettingsPanel>
			...
			<SettingsGroupLabel
				...
				draft?.provider === "s3" && (tested || saved?.provider === "s3") ? (
					<StatusChip label={tested ? "Tested" : "Configured"} dot="var(--green)" />
				) : undefined
			}
			>
				Asset storage
							...
							<SettingsFormRow>
								...
								<SettingsField>
									...
									<Input
										...
										disabled={!!busy}
										autoComplete="off"
										placeholder={draft.secretAccessKeySet ? "Leave blank to keep current key" : "Secret key"}
							...
							<SettingsFormRow>
								...
								<label {...stylex.props(sx.mb3, sx.flex, sx.minH10, sx.itemsCenter, sx.justifyBetween, sx.gap4, sx.fontMedium, sx.textDim, typography.label)}>
									...
									<Switch
										...
										disabled={!!busy}
										onCheckedChange={(forcePathStyle) => patch({ forcePathStyle })}
									/>
								</label>
							</SettingsFormRow>
						</>
			...
			<SettingsHint>
				S3-compatible mode works with AWS S3, Cloudflare R2, and compatible services.
				New assets go directly to the bucket. Existing local assets stay readable.
			</SettingsHint>
		</SettingsPanel>
	);
}
