...
describe("memory v2 runtime migration", () => {
	...
	test("seals a verified import and survives JSON cleanup plus restart", async () => {
		...
		expect(restarted.store.get("legacy-1")?.summary).toBe("A durable imported fact.");
		expect(JSON.parse(restarted.store.legacyRaw("legacy-1") || "{}").by).toBe("Fable");
	});
	...
	test("upgrades an older seal and backfills raw legacy provenance", async () => {
		...
		expect(JSON.parse(result.store.metadata("legacy-migration-v2") || "{}").migrationVersion).toBe(2);
	});
	...
	test("imports JSON written during a legacy rollback before resealing v2", async () => {
		...
		closeMemoryRuntime();
		const restarted = await ensureMemoryV2Ready();
		expect(restarted.store.get("rollback-write")?.summary)
			.toBe("A fact written while legacy mode was active.");
	});
