		...
		return (
			<SettingRow key={id} className={mergeStylexOverrideClassName("", sx.itemsStart)}>
				<SettingRowText>
					...
					{conflicted && (
						<div
							{...stylex.props(sx.mt2, sx.flex, sx.flexWrap, sx.itemsCenter, sx.gapX2, sx.gapY15, sx.textDim, typography.supporting)}
							role="alert"
						>
					...
					{problem?.id === id && (
						<div {...stylex.props(sx.mt15, sx.textRed, typography.supporting)} role="alert">
							{problem.message}
						</div>
					)}
				</SettingRowText>
...
function RecordingPill({ held }: { held: string[] }) {
	return (
		<span
			{...stylex.props(sx.flex, sx.itemsCenter, sx.gap15, sx.roundedControl, sx.bgAccentSoft, sx.px25, sx.py1, sx.textAccent, typography.meta)}
			role="status"
			aria-live="polite"
		>
			{held.length > 0 ? (
