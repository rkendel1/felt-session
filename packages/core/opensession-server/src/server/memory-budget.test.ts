...
const REPO: MemoryScope = { key: "repo-testrepo", kind: "repo", label: "testrepo" };
...
describe("supersede", () => {
	test("a correction archives what it replaces, and the archived entry leaves the prompt", async () => {
		...
		const right = await addSessionMemory(REPO, "The flag is actually --quick.", "tester", {
			supersedes: [wrong.id],
			scopes: SCOPES,
		});
		...
		const all = (await listSessionMemory(SCOPES, { includeArchived: true })).flatMap((s) => s.entries);
	...
	test("a correction can archive an entry in a DIFFERENT scope of the session", async () => {
		const teamFact = await addSessionMemory(TEAM, "Deploys run at noon.", "tester");
	...
	test("an entry cannot supersede itself", async () => {
		...
		expect(activeMemories((await listSessionMemory(SCOPES, { includeArchived: true }))[0].entries)).toHaveLength(1);
	});
});

describe("search", () => {
	test("finds archived entries, so a budget never loses information", async () => {
		const old = await addSessionMemory(REPO, "The postgres socket lives in /var/run.", "tester");
		await addSessionMemory(REPO, "Correction: the postgres socket is in /tmp.", "tester", {
			supersedes: [old.id],
			scopes: SCOPES,
		});
	...
	test("every term must match, so a long entry does not match by accident", async () => {
		await addSessionMemory(REPO, "A long entry mentioning deploy and cache and socket.", "tester");
		expect(await searchSessionMemory(SCOPES, "deploy cache")).toHaveLength(1);
		expect(await searchSessionMemory(SCOPES, "deploy kubernetes")).toHaveLength(0);
	});

	test("archived entries can be excluded", async () => {
		const e = await addSessionMemory(REPO, "retired fact", "tester");
		await archiveMemories(SCOPES, [e.id]);
		expect(await searchSessionMemory(SCOPES, "retired", { includeArchived: false })).toHaveLength(0);
...
describe("budget", () => {
	const big = (n: number, prefix: string) =>
		Array.from({ length: n }, (_, i) =>
			entry(`${prefix}${i}`.padEnd(8, "x"), `${prefix} fact ${i} ${"y".repeat(1000)}`, `2026-01-${String(i + 1).padStart(2, "0")}T00:00:00.000Z`),
		);
...
describe("snapshot", () => {
	test("a session reuses the same bytes even after the store changes", async () => {
		...
		expect(await snapshotMemoryNote("session-b", build)).toContain("Someone else's fact");
	});
	...
	test("this session's own write refreshes its snapshot", async () => {
		...
		expect(await snapshotMemoryNote("session-c", build)).toContain("My own new fact");
	});
