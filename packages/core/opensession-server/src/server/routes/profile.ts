...
import { configuredIdentity, parseTeamMember, type TeamMember } from "../config";
import {
	persistRawConfig,
	rawConfig,
...
export async function handleProfileRoutes(
	...
	const { req, path } = ctx;
	if (path !== "/api/profile" && path !== "/api/profile/image") return undefined;
	...
	if (path === "/api/profile" && req.method === "PUT") {
		...
		return withConfigMutationLock(async () => {
			...
			const team = (
				Array.isArray(identity.team) ? identity.team : []
			).filter(
