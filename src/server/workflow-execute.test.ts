...
function mockRunAgent(scripts: Array<StreamEvent[] | ((opts: RunAgentOpts) => StreamEvent[])>) {
...
describe("validateJsonSchema", () => {
	test("valid nested object passes", () => {
		...
		const value = { name: "a", count: 2, files: ["x", "y"], meta: { ok: true } };
	...
	test("missing required property", () => {
		...
		expect(validateJsonSchema({}, schema)).toEqual(["id: missing required property"]);
	});
	...
	test("enum + const", () => {
		...
		expect(validateJsonSchema(6, { const: 5 })[0]).toContain("expected const 5");
	});
	...
	test("anyOf passes when any branch matches", () => {
		...
		expect(validateJsonSchema(true, schema)).toEqual(["(root): matched no anyOf branch"]);
	});
	...
	test("required Object.prototype key is really required (own properties only)", () => {
		...
		expect(validateJsonSchema({}, schema)).toEqual(["constructor: missing required property"]);
	...
	test("array-form type: nullable idiom accepts any listed type, rejects others", () => {
		...
		expect(validateJsonSchema(5, schema)).toEqual(["(root): expected string | null, got number"]);
	});
...
describe("workflowExecutor", () => {
	...
	test("result capped at maxResultChars", async () => {
		...
		const outcome = await workflowExecutor.execute({ prompt: "p", opts: {}, seq: 0 }, makeCtx());
		expect(outcome.ok).toBe(true);
	...
	test("schema: valid fenced json → structured object", async () => {
		...
		const calls = mockRunAgent([reply('Sure:\n```json\n{"files": ["a.ts", "b.ts"]}\n```')]);
		const outcome = await workflowExecutor.execute(
	...
	test("schema: garbage on every attempt → ok:false after schemaAttempts", async () => {
		...
		expect(outcome.error).toContain(`after ${WORKFLOW_LIMITS.schemaAttempts} attempts`);
	...
	test("semaphore-friendly: concurrent executes don't share state", async () => {
		...
		_setRunAgentForTests(async function* (opts: RunAgentOpts) {
			...
			yield { type: "text_chunk", text: `result-${opts.prompt.slice(-1)}` } as StreamEvent;
