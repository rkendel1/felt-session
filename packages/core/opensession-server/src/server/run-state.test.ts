...
const EVENTS = new Set<RunEvent>(
	STATES.flatMap(
		(s) => Object.keys(RUN_STATE_TRANSITIONS[s]) as RunEvent[],
	),
);
...
describe("transition table shape", () => {
	test("every state has at least one outgoing edge (no dead ends)", () => {
		for (const state of STATES) {
			expect(
				Object.keys(RUN_STATE_TRANSITIONS[state]).length,
			).toBeGreaterThan(0);
		}
	});
...
describe("lifecycle paths", () => {
	test("happy path: prompt → register → turn end", () => {
		expect(walk("idle", ["prompt", "run_registered", "turn_end"])).toBe(
			"idle",
		);
	});
	...
	test("create flow: workspace prep precedes the first prompt", () => {
		expect(walk("idle", ["workspace_prepare", "workspace_ready", "prompt"])).toBe(
			"starting",
		);
	});
