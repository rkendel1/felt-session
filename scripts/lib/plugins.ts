 ...
 */
export function resolveSource(input: string): { url: string } | { error: string } {
	const source = (input || "").trim();
	if (!source) return { error: "give a package to install (owner/repo, a git URL, or a path)" };
	if (source.startsWith("-")) return { error: "a source cannot start with a dash" };
	if (/ext::/i.test(source)) return { error: "the ext:: transport is not allowed" };
...
export async function fetchPackage(
	...
	if (existsSync(join(dir, ".git"))) {
		...
		if (fetched.code !== 0) return { error: fetched.stderr || "git fetch failed" };
		const checkout = await run(["git", "-C", dir, "checkout", "--detach", "FETCH_HEAD"]);
		if (checkout.code !== 0) return { error: checkout.stderr || "git checkout failed" };
	} else {
		...
		if (cloned.code !== 0) return { error: cloned.stderr || "git clone failed" };
	}
...
export function automationKey(automation: { eventKey?: string; name: string }): string {
...
export function recipeFor(
	pkg: string,
	entry: { id: string; label?: string; description?: string; requires?: string[]; automation: any },
): Recipe {
	return {
		id: `${pkg}/${entry.id}`,
...
function owns(owned: InstalledArtifact[], kind: ArtifactKind, ref: string): boolean {
...
export function planInstall(
	...
	for (const feed of manifest.feeds || []) {
		consider("feed", feed.id, state.feeds.includes(feed.id), `${feed.title} via ${feed.items.server}`, feed);
		const server = feed.items.server;
		if (!declaredServers.includes(server) && !state.mcpServers.includes(server)) {
			warnings.push(`feed "${feed.id}" reads from MCP server "${server}", which is not installed`);
		}
	}
	...
	try {
		for (const action of sorted) {
			...
			if (action.kind === "mcp") {
				...
				const hash = stores.addSkill(action.ref, join(dir, String(action.payload)));
...
export function reviewLines(manifest: PackageManifest, plan: InstallPlan): string[] {
	const lines: string[] = [];
	...
	for (const action of plan.actions) {
		lines.push(`${label[action.kind].padEnd(11)} ${action.ref.padEnd(22)} ${action.detail}`);
	}
	for (const stale of plan.removals) {
		lines.push(`${label[stale.kind].padEnd(11)} ${stale.ref.padEnd(22)} removed (no longer in the manifest)`);
	}
	return lines;
}
...
async function listPackages(): Promise<number> {
	...
	for (const pkg of packages) {
		info(`${green(pkg.name.padEnd(20))} ${pkg.version.padEnd(10)} ${dim(pkg.description)}`);
		info(`  ${dim(pkg.source)}${pkg.commit ? dim(` @ ${pkg.commit.slice(0, 8)}`) : ""}`);
...
function confirmPlan(
	...
	if (yes) return true;
	if (!canPrompt()) {
		fail("not a terminal", "re-run with --yes once you have read the plan above");
		return false;
	}
	return askYesNo("Install this package?", false);
}

async function addPackage(source: string, opts: PluginsOptions): Promise<number> {
	...
	if (manifest.requires?.length) {
		info(dim(`  needs: ${manifest.requires.join(", ")} (opensession integrations enable <id>)`));
	}
	if (plan.actions.some((a) => a.kind === "automation")) {
		warn("automations were seeded disabled", "restart to create them, then enable in the UI");
	}
	return 0;
}

async function updatePackage(name: string, opts: PluginsOptions): Promise<number> {
...
export async function plugins(
	...
	if (verb === "add" || verb === "install") return await addPackage(arg ?? "", opts);
	if (verb === "update") return await updatePackage(arg ?? "", opts);
	if (verb === "remove" || verb === "uninstall") return await removePackage(arg ?? "");
	if (!verb || verb === "list") return await listPackages();
	fail(`unknown subcommand '${verb}'`, "usage: opensession plugins [add|update|remove]");
	return 1;
}
