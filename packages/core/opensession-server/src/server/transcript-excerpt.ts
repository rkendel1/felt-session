...
function toolInputLine(e: TranscriptEntry): string {
	...
	for (const key of ["command", "filePath", "file_path", "path", "pattern", "query", "url"]) {
		const v = input[key];
		if (typeof v === "string" && v.trim()) return ` ${key}=${v.replace(/\s+/g, " ").slice(0, 300)}`;
	}
	return "";
}
...
export function formatExcerpt(
	...
	for (const w of ex.windows) {
		...
		for (const e of w.entries) {
			const body = (e.content || "").replace(/[ \t]+/g, " ").trim().slice(0, perEntry);
			const line = `[${e.seq}] ${roleLabel(e)}:${toolInputLine(e)}${body ? ` ${body}` : ""}`;
			if (used + line.length > budget) {
				out.push("… (excerpt budget reached — narrow the query or lower `limit`)");
				stopped = true;
				break;
			}
			out.push(line);
	...
	const foot: string[] = [];
	if (ex.matched)
		foot.push(`${ex.matched} matching entr${ex.matched === 1 ? "y" : "ies"} in this session.`);
	if (ex.lastSeq) foot.push(`session runs to seq ${ex.lastSeq}.`);
	if (ex.truncated) foot.push("older entries beyond the scan window were not searched.");
