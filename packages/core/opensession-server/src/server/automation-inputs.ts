...
export function sanitizeAutomationInputs(
	...
	for (let index = 0; index < value.length; index++) {
		...
		if (!INPUT_ID_RE.test(id)) return { error: `${at}.id must be a short slug` };
		if (ids.has(id)) return { error: `duplicate automation input id "${id}"` };
		ids.add(id);
		const label = String(raw.label || "").trim().slice(0, 120) || undefined;
		...
		if (source.type === "slack_channel") {
			...
			if (!SLACK_CHANNEL_RE.test(channel))
				return { error: `${at}.source.channel must be a Slack C…/G… channel id` };
...
function windowFor(
	...
	const base = mode === "since_last_success" && committed
		? committed
		: latest - minutes * 60;
...
async function collectSlack(
	...
	while (messages.length < limit) {
		...
		if (!response?.ok)
			throw new Error(`Slack conversations.history: ${response?.error || "unknown error"}`);
	...
	if (input.source.includeThreads) {
		const roots = messages.filter((message) => Number(message.reply_count) > 0).slice(0, 30);
		for (const root of roots) {
			...
			for (const reply of response.messages || []) {
				if (reply.ts !== root.ts) messages.push({ ...reply, __threadTs: root.ts });
			}
		}
	}
	...
	const unique = [...new Map(messages.map((message) => [message.ts, message])).values()]
		.filter((message: any) => {
			...
			if (Number(message.ts) < window.oldest || Number(message.ts) > window.latest)
				return false;
			if (!input.source.includeBots && (message.bot_id || message.subtype === "bot_message"))
				return false;
			return !message.subtype || message.subtype === "bot_message";
		})
		.sort((a: any, b: any) => Number(a.ts) - Number(b.ts));
	const userIds = [...new Set(unique.map((message: any) => message.user).filter(Boolean))];
...
function collectReports(
	...
	const sourceId = input.source.automationId === "self"
		? automationId
	...
	return {
		label: input.label || (sourceId === automationId ? "Previous reports" : `Reports ${sourceId}`),
...
async function reduceInput(
	...
	if (!collected.records.length) return { text: "No new records in this window." };
	...
	for (const [index, data] of inputChunks.slice(0, 6).entries()) {
		...
		if (!result) {
			return {
				...
				warning: "Haiku reduction failed; a bounded raw excerpt was supplied instead.",
			};
		}
		reduced.push(result);
	...
	return {
		...
		...(inputChunks.length > 6
			? { warning: "Input exceeded the six-chunk reduction cap; later records were omitted." }
			: {}),
	};
}
	...
	for (const input of opts.inputs) {
		try {
			const collected = isSlackInput(input)
				? await collectSlack(input, state.inputs[input.id], opts.startedAt, deps)
