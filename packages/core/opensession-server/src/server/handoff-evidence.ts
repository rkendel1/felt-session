...
function callSubject(e: TranscriptEntry | undefined): string {
	...
	if (input && typeof input === "object") {
		for (const key of ["command", "filePath", "file_path", "path", "pattern", "url"]) {
			const v = input[key];
...
export async function collectHandoffEvidence(
	...
	if (session.lastRunError?.message) ev.lastRunError = session.lastRunError.message;
...
export function formatHandoffEvidence(
	...
	const lines: string[] = [
		opts.title ?? `— evidence for \`${ev.sessionId}\` (computed by the server, not written by the worker) —`,
	];
	const meta = [`state: ${ev.state}`];
	if (ev.branch) meta.push(`branch ${ev.branch}`);
	if (ev.usage) meta.push(`${ev.usage.turns} turn(s)`);
	lines.push(meta.join(" · "));
	if (ev.lastRunError) lines.push(`run error: ${ev.lastRunError.slice(0, 300)}`);
	if (ev.pr) lines.push(`PR: ${ev.pr.state ? `${ev.pr.state} ` : ""}${ev.pr.url}`);
	...
	if (ev.diff) {
		...
		for (const f of ev.diff.files)
			lines.push(`  ${f.status.slice(0, 1).toUpperCase()} ${f.path} (+${f.additions}/-${f.deletions})`);
	...
	if (ev.failures.length) {
		...
		for (const f of ev.failures) {
			lines.push(`  ✗ ${f.tool}: ${f.what}${f.laterPassed ? "  [a later attempt got past this]" : ""}`);
...
export async function notifyParentOfFailedRun(
	...
	try {
		...
		const parentId = typeof file?.parentSessionId === "string" ? file.parentSessionId : "";
		...
		if (since(file.lastReportToParentAt) < REPORT_GRACE_MS) return "already-reported";
