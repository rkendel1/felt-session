      ...
      {images.map((src, i) => (
        <div key={i} {...stylex.props(sx.relative, sx.leading0)}>
          <button
            ...
            {...stylex.props(sx.focusRing, sx.block, sx.cursorZoomIn, sx.roundedControl, sx.leading0)}
            onClick={(event) =>
              openLightbox(
          ...
          >
            <img
              src={src}
              alt=""
              {...stylex.props(sx.h14, sx.wAuto, sx.maxW120px, sx.roundedControl, sx.border, sx.borderLine60, sx.objectCover)}
            />
          </button>
          <button
            type="button"
