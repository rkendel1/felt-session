...
async function command(argv: string[], timeoutMs = 120_000): Promise<CommandResult> {
...
async function qualifyDocker(): Promise<void> {
  if (!Bun.which("docker")) {
    throw Object.assign(new Error("Docker is not installed"), { code: "DOCKER_NOT_INSTALLED" });
  }
  const info = await command(["docker", "info", "--format", "{{.ServerVersion}}"], 30_000);
  if (info.exitCode !== 0) {
  ...
  if (inspect.exitCode !== 0) {
    throw Object.assign(new Error("Open Session sandbox image is not installed"), {
  ...
  try {
    ...
    if (run.exitCode !== 0) throw new Error("Docker qualification container failed to start");
    ...
    if (probe.exitCode !== 0) throw new Error("Docker qualification command failed");
    const commit = await command(["docker", "commit", source, snapshot], 180_000);
    if (commit.exitCode !== 0) throw new Error("Docker qualification snapshot failed");
    ...
    if (restored.exitCode !== 0) {
      throw new Error("Docker qualification snapshot did not restore filesystem state");
    }
  } finally {
    await command(["docker", "rm", "-f", source], 30_000).catch(() => undefined);
    await command(["docker", "image", "rm", "-f", snapshot], 30_000).catch(() => undefined);
  }
}
...
function failureCode(error: unknown): string {
  ...
  if (message.includes("unauthor") || message.includes("token") || message.includes("api key")) {
    return "CREDENTIAL_REJECTED";
  }
  if (message.includes("quota") || message.includes("limit")) return "PROVIDER_QUOTA";
  if (message.includes("snapshot") || message.includes("image")) return "SNAPSHOT_FAILED";
...
function failureSummary(code: string): string {
  const summaries: Record<string, string> = {
    ...
    INGRESS_HEALTH_FAILED: "Caddy is not routing sandbox ingress health to port 3860.",
    INGRESS_WEBSOCKET_FAILED: "Caddy did not complete an authenticated sandbox WebSocket upgrade.",
    INGRESS_TIMEOUT: "The public sandbox ingress did not respond in time.",
    CREDENTIAL_REJECTED: "The provider rejected the workspace credentials. Replace them and retry.",
    PROVIDER_QUOTA: "The provider account has insufficient quota for a disposable test sandbox.",
    SNAPSHOT_FAILED: "The provider could not restore a distinct qualification snapshot.",
