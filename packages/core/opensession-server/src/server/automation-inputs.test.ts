		...
		const deps = {
			slackApiGet: async (_method: string, args: Record<string, any>) => {
				...
				return {
					ok: true,
					messages: [
						{ type: "message", ts: "1786361400.000001", user: "U123456", text: "A concrete claim" },
					],
					response_metadata: {},
				};
			},
			resolveSlackUser: async () => ({ name: "Alice" }),
			oneShot: async () => "- Alice made a concrete claim — slack://C01ED50A2KG/1786361400.000001?message=1786361400.000001",
		};
