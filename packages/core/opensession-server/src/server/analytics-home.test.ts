...
test("Home takes activity from audit and tokens from engine usage", async () => {
	...
	expect(loadedDates).toEqual([
		Array.from({ length: 15 }, (_, back) => new Date(NOW - back * 86_400_000).toISOString().slice(0, 10)),
	]);
	...
	expect(stats.completeWeek).toMatchObject({ sessions: 1, turns: 1, inputTokens: 10, outputTokens: 20 });
});
