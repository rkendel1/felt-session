...
const sx = stylex.create({
	...
	smoothShadowMd: {
			boxShadow: "0 2px 6px -2px var(--smooth-shadow-color), 0 10px 28px -8px var(--smooth-shadow-color)"
	},
	px35: {
			paddingInline: "14px"
	},
  ...
  return (
    ...
    <div ref={hostRef} {...stylex.props(sx.contents)} onMouseUp={onMouseUp}>
      ...
      {sel && send && (
        ...
        >
          {sent ? (
            <div {...stylex.props(sx.px35, sx.py2, sx.whitespaceNowrap, sx.textAccent, typography.label)}>
              Sent to session ✓
            </div>
          ) : composing ? (
            <div {...stylex.props(sx.flex, sx.flexCol, sx.gap2, sx.p25)}>
              <div {...stylex.props(sx.maxH16, sx.overflowYAuto, sx.borderL2, sx.borderLineStrong, sx.pl2, sx.breakWords, sx.whitespacePreWrap, sx.textFaint, typography.supporting)}>
                {sel.text}
              </div>
              <textarea
                autoFocus
