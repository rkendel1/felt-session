...
export function utilityClassName(value: string): string {
	...
	// Uncompiled StyleX (bun test, dev) returns inline styles and no class name.
	// Keep the source spelling — residual selectors and tests read it — and add
	// a value-derived suffix so the registration key stays unique to this exact
	// utility string. Compiled builds have a class name and never reach this.
	let hash = 2166136261;
	for (const char of value)
		hash = Math.imul(hash ^ char.charCodeAt(0), 16777619);
	const marker =
		props.className ?? `${value} sx-compat-${(hash >>> 0).toString(36)}`;
	const result = clsx(classNames, marker);
