...
function cleanStrings(value: unknown, max = 64): string[] {
	...
	return [...new Set(value.map(String).map((v) => v.trim()).filter((v) => v && v.length <= max))].slice(0, 64);
}
...
function normalizeResources(value: unknown): RunnerResources | undefined {
	if (!value || typeof value !== "object" || Array.isArray(value)) return undefined;
	const raw = value as Record<string, unknown>;
	const number = (key: string) => typeof raw[key] === "number" && Number.isFinite(raw[key]) && raw[key] >= 0 ? raw[key] : undefined;
	...
	if (raw.gpu && typeof raw.gpu === "object" && !Array.isArray(raw.gpu)) {
		...
		const kind = ["nvidia", "amd", "apple", "intel", "other"].includes(String(candidate.kind))
			? String(candidate.kind) as NonNullable<RunnerResources["gpu"]>["kind"]
			: "other";
		gpu = {
			kind,
			...(typeof candidate.model === "string" && candidate.model.length <= 160 ? { model: candidate.model } : {}),
			...(typeof candidate.vramGb === "number" && candidate.vramGb >= 0 ? { vramGb: candidate.vramGb } : {}),
			...(typeof candidate.driver === "string" && candidate.driver.length <= 120 ? { driver: candidate.driver } : {}),
			...(typeof candidate.cuda === "string" && candidate.cuda.length <= 80 ? { cuda: candidate.cuda } : {}),
			...(typeof candidate.rocm === "string" && candidate.rocm.length <= 80 ? { rocm: candidate.rocm } : {}),
			...(typeof candidate.metal === "boolean" ? { metal: candidate.metal } : {}),
		};
	}
	const result = {
		...(number("cpuCores") !== undefined ? { cpuCores: number("cpuCores") } : {}),
		...(number("memoryGb") !== undefined ? { memoryGb: number("memoryGb") } : {}),
...
function normalizeRunner(value: Runner): Runner {
	...
	const platform: RunnerPlatform = ["darwin", "linux", "win32"].includes(String(raw.platform)) ? raw.platform as RunnerPlatform : "linux";
...
const pairings: Map<string, Pairing> = (g.__opensessionRunnerPairings ??= new Map()) as Map<string, Pairing>;
...
export function createRunnerPairing(createdBy?: string): { code: string; expiresAt: number } {
...
export function listRunnerPairings(): Pairing[] {
	for (const [code, value] of pairings) if (value.expiresAt <= Date.now()) pairings.delete(code);
...
export function publicRunner(runner: Runner, online: boolean, busy = false): Omit<Runner, "tokenHash"> & { state: RunnerState } {
	...
	return { ...rest, ...(activeReservation ? { reservation: activeReservation } : {}), state: runner.maintenance ? "maintenance" : busy ? "busy" : online ? "online" : "offline" };
}
