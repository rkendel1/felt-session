...
export class VimEngine {
	...
	private handleNormal(e: VimKeyEvent, doc: VimDoc): VimResult | null {
		...
		switch (k) {
			...
			case "h":
				return this.motionTo(doc, Math.max(lineStartAt(t, caret), caret - count), false);
			case "l":
				return this.motionTo(doc, Math.min(lineEndAt(t, caret), caret + count), false);
			case "j":
			case "k":
			...
			case "w":
				...
				// behaves like ce.
				if (this.op === "c")
					return this.motionTo(doc, this.repeat(t, caret, count, wordEndBoundary), false);
				return this.motionTo(doc, this.repeat(t, caret, count, nextWordStart), false);
			case "b":
				return this.motionTo(doc, this.repeat(t, caret, count, prevWordStart), false);
			case "e":
				return this.motionTo(doc, this.repeat(t, caret, count, wordEndBoundary), false);
			case "0":
		...
		switch (k) {
			...
			case "o": {
				...
				this.mode = "insert";
				this.clearPending();
				return { text: t.slice(0, le) + "\n" + t.slice(le), start: le + 1, end: le + 1, mode: "insert" };
			}
			case "O": {
				this.pushUndo(doc);
				...
				this.mode = "insert";
				this.clearPending();
				return { text: t.slice(0, ls) + "\n" + t.slice(ls), start: ls, end: ls, mode: "insert" };
			}
			case "x": {
				const le = lineEndAt(t, caret);
				if (caret >= le) return this.result(t, caret);
			...
			case "s": {
				const le = lineEndAt(t, caret);
				return this.applyOperatorAs(doc, "c", caret, Math.min(le, caret + count));
			}
			case "S":
			...
			case "V": {
				...
				this.anchor = caret;
				this.clearPending();
				return { text: t, start: lineStartAt(t, caret), end: lineEndAt(t, caret), mode: "visual-line" };
			}
		}
	...
	private handleVisual(e: VimKeyEvent, doc: VimDoc): VimResult | null {
		...
		switch (k) {
			...
			case "w":
				return this.visualSelect(doc, this.repeat(t, head, count, nextWordStart));
			case "b":
				return this.visualSelect(doc, this.repeat(t, head, count, prevWordStart));
			case "e":
				return this.visualSelect(doc, this.repeat(t, head, count, wordEndBoundary));
			case "0":
			...
			case "o":
				this.anchor = head;
				return this.visualSelect(doc, this.anchor === doc.start ? doc.end : doc.start);
			case "v":
				if (line) {
					this.mode = "visual";
					return this.visualSelect(doc, head);
			...
			case "y": {
				const [from, to] = this.visualRange(doc);
				this.toNormal();
				return this.applyOperatorAs({ ...doc, start: from, end: from }, "y", from, to, line);
			}
			case "p": {
				const [from, to] = this.visualRange(doc);
				this.pushUndo({ ...doc, start: from, end: from });
				const reg = this.regLinewise ? this.register.replace(/\n$/, "") : this.register;
	...
	private vertTarget(t: string, pos: number, delta: number): number | null {
		...
		if (delta > 0) {
			for (let i = 0; i < delta; i++) {
				...
				if (le >= t.length) return i === 0 ? null : Math.min(targetLs + col, le);
				targetLs = le + 1;
			}
		} else {
			for (let i = 0; i < -delta; i++) {
				if (targetLs === 0) return i === 0 ? null : Math.min(targetLs + col, lineEndAt(t, targetLs));
	...
	private applyOperator(doc: VimDoc, from: number, target: number, linewise: boolean): VimResult {
		const op = this.op!;
		this.clearPending();
	...
	private setRegister(content: string, linewise: boolean) {
		this.register = linewise && !content.endsWith("\n") ? content + "\n" : content;
	...
	private visualRange(doc: VimDoc): [number, number] {
		...
		if (doc.start === doc.end) return [doc.start, Math.min(doc.text.length, doc.end + 1)];
