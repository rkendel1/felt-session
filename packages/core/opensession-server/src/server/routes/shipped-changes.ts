...
export async function handleShippedChangeRoutes(
	...
	if (!match || (req.method !== "GET" && req.method !== "POST" && req.method !== "PUT"))
		return;
	...
	if (req.method === "PUT") {
		...
		if (!share || !share.ts)
			return Response.json({ error: "That message can no longer be undone" }, { status: 409 });
		if (!slackToken)
			return Response.json(
	...
	try {
		...
		const share: SessionSlackShare | undefined = result.channel && {
			...
			...(result.announcementKey ? { announcementKey: result.announcementKey } : {}),
		};
		if (share) {
			await updateSessionFile(session.id, (data) => ({
				...data,
