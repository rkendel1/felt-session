...
7. For a confirmed finding, branch off origin/${repo.defaultBranch}: \`git checkout -b deepsec-scan-<short-slug> origin/${repo.defaultBranch}\`. Write a MINIMAL fix targeting the root cause only.
   ...
   - If you are NOT confident in a safe fix (e.g. it touches core auth broadly), do NOT guess — open the PR with the finding writeup + recommended fix and NO code change, and add "[needs-human-fix]" to the title.
8. Commit, push, then \`gh pr create --repo ${repo.ghRepo} --base ${repo.defaultBranch} --title "[deepsec][scan] <concise title>" --body "<finding, impact, fix, severity; note: generated by an on-demand ${productName()} security scan via vercel-labs/deepsec>"\`. One PR per finding. NEVER run \`gh pr merge\`.

## Finish
9. End with a concise summary in your final message: # findings by severity, PR links, and any findings you dropped as false positives (one line each on why). If the scan errored (e.g. claude CLI not logged in), report the error clearly rather than half-finishing.
...
- Keep each PR small and focused; deduplicate if two findings share one root cause.${scanExtras(profile, instructions)}`;
}
...
export function buildInteractivePrompt(
  ...
  return `We're running a deepsec security scan on ${repo.id} (${repo.ghRepo}) together — interactive mode. Instead of scanning end to end, collaborate with me to tailor the threat model before you run anything.
...
- Severity bar and what to do with findings (PR per finding vs. report only).
