...
const SHEET = (process.argv.find((a) => a.endsWith(".css")) as string | undefined) ?? "/tmp/tw-current.css";
...
function parseDecls(body: string): Decl[] {
	...
	for (const part of body.split(";")) {
		...
		if (i < 0) continue;
		out.push({ prop: part.slice(0, i).trim(), value: part.slice(i + 1).trim() });
	}
	return out;
}
...
function translateRule(decls: Decl[]): { ok: StyleObj; good: boolean } {
	...
	for (const d of decls) {
		...
		let value = d.value.replace(/var\((--tw-[a-z-]+)(?:,([^()]*))?\)/g, (_m, v: string, fb?: string) => {
...
const TYPE_ROLES: Record<string, string> = {
	"text-meta": "meta", "text-label": "label", "text-supporting": "supporting",
	"text-control-label": "controlLabel", "text-body": "body",
	"text-item-title": "itemTitle", "text-dialog-title": "dialogTitle",
	"text-section-title": "sectionTitle", "text-page-title": "pageTitle",
	"text-stat": "stat",
};

/** Expressible variants: name → how the StyleX key is formed. */
type VariantKey = { kind: "pseudo"; key: string } | { kind: "media"; query: string };
...
function variantToKey(name: string): VariantKey | null {
	if (PSEUDO_VARIANTS[name]) return { kind: "pseudo", key: PSEUDO_VARIANTS[name] };
	...
	if (arb) {
		return {
			kind: "media",
			query: arb[1] === "max" ? `(max-width: ${Number(arb[2]) - 1}px)` : `(min-width: ${arb[2]}px)`,
		};
	}
	void normalizeQuery;
	return null;
...
function mergeInto(obj: StyleObj, path: Array<VariantKey>, style: StyleObj): boolean {
	let cur: any = obj;
	for (const k of path) {
...
function tokenToStyle(token: string, variants: string[]): TokenStyle | null | "residual" {
...
const report = { files: 0, converted: 0, elements: 0, skippedDynamic: 0, unknownTokens: new Map<string, number>() };

function relImport(fromFile: string, target: string): string {
	const rel = relative(join(FRONTEND, "."), target).replace(/\.ts$/, "");
	const depth = relative(fromFile.split("/").slice(0, -1).join("/"), "").split("/").length;
	const prefix = "./".repeat(1);
	void depth; void prefix;
...
for (const file of files) {
	...
	while ((m = attrRe.exec(srcText))) {
		hits.push({ start: m.index, end: m.index + m[0].length, full: m[0], classes: m[1].split(/\s+/).filter(Boolean) });
	}
	...
	if (fileUnknown.length > 0 && !WRITE) {
		console.log(`${relative(FRONTEND, file)}: ${fileUnknown.length} unknown tokens, skipping in dry-run`);
	}
	...
	const anchor =
		out.split("\n").slice(0, importEnd + 1 + importLines.length).join("\n").length + 1;

	const entries = [...usedStyles.entries()].map(([tok, obj]) => {
		const json = JSON.stringify(obj, null, "\t\t").replace(/\n/g, "\n\t");
...
const top = [...report.unknownTokens.entries()].sort((a, b) => b[1] - a[1]).slice(0, 40);
