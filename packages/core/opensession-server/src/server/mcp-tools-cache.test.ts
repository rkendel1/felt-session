...
describe("toolsCacheKey", () => {
	...
	test("changes when any part of the config changes", () => {
		...
		expect(toolsCacheKey(base)).not.toBe(toolsCacheKey({ ...base, args: ["-y", "srv@1.1.0"] }));
		expect(toolsCacheKey(base)).not.toBe(toolsCacheKey({ ...base, env: { TOKEN: "x" } }));
	});

	test("array order is significant (args are positional)", () => {
		expect(toolsCacheKey({ args: ["a", "b"] })).not.toBe(toolsCacheKey({ args: ["b", "a"] }));
	});
});
...
describe("read/write round trip", () => {
	...
	test("a different config hash misses, so a changed server re-lists", () => {
		...
		expect(readCachedTools("acme", toolsCacheKey({ command: "bunx" }))).toBeUndefined();
	});
