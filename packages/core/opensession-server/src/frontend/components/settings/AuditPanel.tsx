		...
		<SettingsPanel>
			...
			<div {...stylex.props(sx.mb3, sx.flex, sx.flexWrap, sx.itemsCenter, sx.gap2, sx.px5)}>
				...
				<Select
					...
					onChange={setType}
				/>
				<label {...stylex.props(sx.flex, sx.cursorPointer, sx.itemsCenter, sx.gap2, sx.textDim, typography.label)}>
					...
					return (
						<div key={i} {...stylex.props(expanded === i && sx.bgPressed)}>
							...
							>
								...
								<span {...stylex.props(sx.textFg, sx.fontMedium, sx.shrink0)}>{t}</span>
								{e.run_kind ? <span {...stylex.props(sx.textFaint, sx.shrink0)}>{String(e.run_kind)}</span> : null}
								<span {...stylex.props(sx.textDim, sx.truncate)}>{auditSummary(e)}</span>
								{sid && (
									<a
										{...stylex.props(sx.mlAuto, sx.shrink0, sx.textFaint, sx.underline, typography.meta)}
							...
							{expanded === i && (
								<pre {...stylex.props(sx.m0, sx.overflowXAuto, sx.borderT, sx.borderLine, sx.px5, sx.py25, sx.leadingRelaxed, sx.textDim, typography.meta)}>
