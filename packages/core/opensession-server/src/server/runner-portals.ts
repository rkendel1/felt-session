...
const g = globalThis as { __opensessionRunnerPortalRelays?: Map<string, RunnerPortalRelay> };
const relays = (g.__opensessionRunnerPortalRelays ??= new Map());
type BrowserPortalSocket = { ws: any; record: RunnerPortalRecord };
const socketState = globalThis as { __opensessionRunnerPortalSockets?: Map<string, BrowserPortalSocket>; __opensessionRunnerPortalFramesInstalled?: boolean };
const browserSockets = (socketState.__opensessionRunnerPortalSockets ??= new Map());
...
function relayScope(record: RunnerPortalRecord): string { return `runner-${record.runnerId}-${record.sessionId}`; }
function relayHttpsPort(record: RunnerPortalRecord): number { return sandboxHttpsPortFor(relayScope(record), record.port); }

function safeRequestHeaders(headers: Headers): Record<string, string> {
	const forwarded: Record<string, string> = {};
	for (const [name, value] of headers) if (!HOP_HEADERS.has(name.toLowerCase()) && value.length <= 8_192) forwarded[name] = value;
	return forwarded;
}

function relaySocketFrame(runnerId: string, message: Record<string, unknown>): void {
	...
	if (message.t === "portal_ws_event") {
		try {
			if (message.binary === true && typeof message.data === "string") socket.ws.send(Buffer.from(message.data, "base64"));
...
async function relayResponse(record: RunnerPortalRecord, request: Request): Promise<Response> {
	const session = findSession(record.sessionId);
	if (!session?.runner || session.runner.id !== record.runnerId) return new Response("Portal is no longer available", { status: 404 });
	let body = "";
	if (request.method !== "GET" && request.method !== "HEAD") {
		const bytes = new Uint8Array(await request.arrayBuffer());
		if (bytes.byteLength > MAX_RELAY_BODY) return new Response("Portal request is too large", { status: 413 });
		body = Buffer.from(bytes).toString("base64");
	}
	try {
		const result = await requestRunnerPortal(record.runnerId, {
			sessionId: record.sessionId, repo: record.repo, workspacePath: record.workspacePath, operation: "http", user: session.startedBy || undefined,
			payload: { port: record.port, method: request.method, path: new URL(request.url).pathname + new URL(request.url).search, headers: safeRequestHeaders(request.headers), ...(body ? { body } : {}) },
		}) as { status?: unknown; headers?: unknown; body?: unknown };
		const status = typeof result.status === "number" && Number.isInteger(result.status) ? result.status : 502;
		const headers = new Headers();
		if (result.headers && typeof result.headers === "object" && !Array.isArray(result.headers)) {
			for (const [name, value] of Object.entries(result.headers as Record<string, unknown>)) if (!HOP_HEADERS.has(name.toLowerCase()) && typeof value === "string") headers.set(name, value);
		}
		const responseBody = typeof result.body === "string" ? Buffer.from(result.body, "base64") : undefined;
		if (responseBody && responseBody.byteLength > 10 * 1024 * 1024) return new Response("Runner Portal response is too large", { status: 502 });
		return new Response(responseBody, { status, headers });
	} catch (error) {
		return new Response(error instanceof Error ? error.message : "Runner Portal relay failed", { status: 502 });
	}
}
...
async function ensureRelay(record: RunnerPortalRecord): Promise<string | null> {
	...
	if (!relay) {
		...
		const server = Bun.serve<{ connectionId: string; record: RunnerPortalRecord; path: string; headers: Record<string, string> }>({
			hostname: "127.0.0.1",
			port: 0,
			fetch(request, relayServer) {
				if (request.headers.get("upgrade")?.toLowerCase() === "websocket") {
					...
					return relayServer.upgrade(request, { data: { connectionId, record: portalRecord, path: url.pathname + url.search, headers: safeRequestHeaders(request.headers) } }) ? undefined : new Response("WebSocket upgrade failed", { status: 400 });
				}
				return relayResponse(portalRecord, request);
			},
			websocket: {
				open(ws) {
					browserSockets.set(ws.data.connectionId, { ws, record: ws.data.record });
					const request = ws.data as { connectionId: string; record: RunnerPortalRecord; path: string; headers: Record<string, string> };
					if (!sendRunnerPortalFrame(request.record.runnerId, {
						t: "portal_ws_open", connectionId: request.connectionId, sessionId: request.record.sessionId,
						repo: request.record.repo, workspacePath: request.record.workspacePath, port: request.record.port, path: request.path, headers: request.headers,
					})) {
						browserSockets.delete(request.connectionId); try { ws.close(); } catch {}
					}
				},
				message(ws, message) {
					const request = ws.data as { connectionId: string; record: RunnerPortalRecord };
					const binary = typeof message !== "string";
					const data = binary ? Buffer.from(message as any).toString("base64") : message;
					if (!sendRunnerPortalFrame(request.record.runnerId, { t: "portal_ws_send", connectionId: request.connectionId, binary, data })) {
						try { ws.close(); } catch {}
					}
				},
				close(ws) {
					...
					sendRunnerPortalFrame(request.record.runnerId, { t: "portal_ws_close", connectionId: request.connectionId });
				},
			},
		});
		relay = { server, record };
	...
	return ensureAuthenticatedPortalRoute(relayHttpsPort(record), `127.0.0.1:${relay.server.port}`);
}
...
export async function runnerPortalUrl(record: RunnerPortalRecord): Promise<string | null> {
	return record.state === "awake" ? ensureRelay(record) : null;
}

export async function runnerPortalPreviewStatus(session: UnifiedSession, user?: string) {
	const portals = await listRunnerPortalServices(session, user);
	const services = await Promise.all(portals.map(async (portal) => ({
		name: portal.name,
		key: portal.key,
		port: portal.port,
...
async function dropRecords(records: readonly RunnerPortalRecord[]): Promise<void> {
	...
	for (const record of records) await dropAuthenticatedPortalRoute(relayHttpsPort(record));
	for (const scope of new Set(records.map(relayScope))) {
		for (const httpsPort of releaseSandboxPreviewPorts(scope)) await dropAuthenticatedPortalRoute(httpsPort);
	}
}
...
function runnerSession(session: UnifiedSession, user?: string) {
	...
	if (!target || !session.repo || !session.worktreeDir) throw new Error("This session does not run on a Runner.");
	const runner = getRunner(target.id);
	if (!runner || !runnerAllowed(runner, { user, repo: session.repo, permission: "portals" })) throw new Error("This Runner is not permitted to expose Portals.");
	return { runner, target, repo: session.repo, workspacePath: session.worktreeDir };
}

function parseRecord(value: unknown, session: UnifiedSession, user?: string): RunnerPortalRecord {
	...
	const command = typeof valueRecord.command === "string" ? valueRecord.command : "";
	const port = typeof valueRecord.port === "number" ? valueRecord.port : NaN;
	if (!NAME.test(name) || !command || !Number.isInteger(port) || port < 1024 || port > 19_000) throw new Error("Runner returned an invalid Portal record.");
	const state = ["starting", "awake", "sleeping", "waking", "failed", "stopped"].includes(String(valueRecord.state)) ? valueRecord.state as PortalRecord["state"] : "failed";
	return {
		name,
		key: typeof valueRecord.key === "string" ? valueRecord.key : `PORTAL_${name.toUpperCase().replace(/-/g, "_")}_PORT`,
		command,
		port,
		state,
		...(typeof valueRecord.description === "string" ? { description: valueRecord.description } : {}),
		...(typeof valueRecord.defaultPath === "string" ? { defaultPath: valueRecord.defaultPath } : {}),
		...(typeof valueRecord.pid === "number" ? { pid: valueRecord.pid } : {}),
		...(typeof valueRecord.startedAt === "string" ? { startedAt: valueRecord.startedAt } : {}),
		...(typeof valueRecord.lastError === "string" ? { lastError: valueRecord.lastError } : {}),
...
export async function startRunnerPortal(input: { session: UnifiedSession; user?: string; name: string; command: string; port?: number; description?: string }): Promise<RunnerPortalRecord> {
	const context = runnerSession(input.session, input.user);
	const allocation = await requestRunnerPortal(context.target.id, {
		sessionId: input.session.id, repo: context.repo, workspacePath: context.workspacePath, operation: "allocate", user: input.user,
		payload: { port: input.port },
	}) as { port?: unknown };
	const port = typeof allocation.port === "number" && Number.isInteger(allocation.port) ? allocation.port : NaN;
	if (!Number.isInteger(port) || port < 1024 || port > 19_000) throw new Error("Runner returned an invalid Portal port.");
	const name = input.name.trim().toLowerCase();
	if (!NAME.test(name) || !input.command.trim()) throw new Error("Invalid Portal service.");
...
export async function listRunnerPortalServices(session: UnifiedSession, user?: string): Promise<RunnerPortalRecord[]> {
	const context = runnerSession(session, user);
	try {
