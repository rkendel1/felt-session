						...
						showInstallHelp ? (
							<span {...stylex.props(sx.flex, sx.minW0, sx.itemsCenter, sx.gap1)}>
								<Button
									...
									className={mergeStylexOverrideClassName("", sx.Ml1, sx.size7, sx.shrink0)}
...
export function DownloadAppsBody({
	...
	if (showInstallHelp)
		return (
			<div {...mergeStylexProps("desktop:grid-cols-3", sx.grid, sx.minH0, sx.flex1, sx.gap3)} >
	...
	return (
		<div {...mergeStylexProps("desktop:grid-cols-[3fr_2fr]", sx.grid, sx.minH0, sx.flex1, sx.gap4)} >
			<AppCard
				preview={
					<div
						{...stylex.props(sx.relative, sx.hFull, sx.overflowHidden, sx.bgCover, sx.bgCenter, sx.pl5, sx.pt5)}
						style={{
					...
					>
						<img
							...
							{...mergeStylexProps("outline outline-1 outline-black/10", sx.hFull, sx.wFull, sx.roundedTlLg, sx.objectCover, sx.objectLeftTop, sx.OutlineOffset1)}
						/>
						<div {...mergeStylexProps("bg-gradient-to-b from-transparent to-surface", sx.pointerEventsNone, sx.absolute, sx.insetX0, sx.bottom0, sx.h13)}  />
					</div>
				}
			...
			<AppCard
				preview={
					<div
						{...stylex.props(sx.relative, sx.flex, sx.hFull, sx.justifyCenter, sx.overflowHidden, sx.bgCover, sx.bgCenter, sx.px3, sx.pt6)}
						style={{
							backgroundImage: `url(${BASE_PATH}/${backgroundName}.webp)`,
						}}
					>
						<img
							...
							{...mergeStylexProps("smooth-shadow-lg", sx.relative, sx.z10, sx.h130, sx.wAuto, sx.maxWNone, sx.originTop, sx.rounded2xl, sx.objectContain, sx.objectTop)}
						/>
						<div {...mergeStylexProps("bg-gradient-to-b from-transparent to-surface", sx.pointerEventsNone, sx.absolute, sx.insetX0, sx.bottom0, sx.z20, sx.h13)}  />
					</div>
				}
				title="Web"
...
function AppCard({
	...
	return (
		<section {...mergeStylexProps("desktop:h-[22rem]", sx.flex, sx.h20rem, sx.flexCol, sx.overflowHidden, sx.roundedCalc22pxVarRf, sx.bgSurface)} >
			<div {...stylex.props(sx.minH0, sx.flex1)}>{preview}</div>
			<div {...mergeStylexProps("desktop:px-5 desktop:pb-5", sx.relative, sx.z10, sx.flex, sx.shrink0, sx.flexCol, sx.px4, sx.pb4)} >
				<h3 {...mergeStylexProps("m-0", sx.fontSemibold, sx.leadingTight, sx.textFg, typography.sectionTitle)} >{title}</h3>
				<p {...stylex.props(sx.mb4, sx.mt1, sx.fontMedium, sx.textDim, typography.body)}>{subtitle}</p>
				{children}
			</div>
		</section>
	);
...
function InstallStep({
	...
	return (
		<section {...mergeStylexProps("desktop:min-h-60", sx.flex, sx.minH48, sx.flexCol, sx.roundedXl, sx.bgPanel, sx.p5)} >
			<div {...stylex.props(sx.mbAuto, sx.flex, sx.size10, sx.itemsCenter, sx.justifyCenter, sx.roundedControl, sx.bgAccent, sx.fontSemibold, sx.textOnAccent, typography.body)}>
				{number}
			</div>
			<h3 {...stylex.props(sx.mb1, sx.mt6, sx.fontSemibold, sx.leadingTight, sx.textFg, typography.sectionTitle)}>{title}</h3>
			<p {...mergeStylexProps("m-0", sx.fontNormal, sx.leadingRelaxed, sx.textDim, typography.body)} >{children}</p>
		</section>
	);
}
