...
function keyForMember(member: TeamMember): string {
	if (member.github?.trim()) return `github-${member.github.trim().toLowerCase()}`;
...
function keyFor(user: string): string | null {
	...
	const slug = trimmed.toLowerCase().replace(/[^a-z0-9@._-]+/g, "-").slice(0, 64);
...
const store = userStore<Profile>({
	...
	clean: (raw) => {
		const image =
			raw && typeof raw === "object" && typeof (raw as Profile).image === "string"
