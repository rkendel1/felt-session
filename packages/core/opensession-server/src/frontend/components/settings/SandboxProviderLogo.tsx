...
function SandboxProviderTile({
	...
	return (
		...
		>
			{children}
			<span
				{...stylex.props(sx.pointerEventsNone, sx.absolute, sx.inset0, sx.roundedLg)}
				style={tileStyle}
				aria-hidden="true"
			/>
		</span>
...
export function SandboxProviderLogo({ provider }: { provider: SandboxProvider }) {
	if (provider === "box") {
		return (
			<SandboxProviderTile className={mergeStylexOverrideClassName("", sx.bgWhite, sx.text111)}>
				<svg viewBox="-4.6 0 35.5 35.5" {...stylex.props(sx.h8, sx.w7)} fill="currentColor" aria-hidden="true">
	...
	if (provider === "daytona") {
		return (
			<SandboxProviderTile className={mergeStylexOverrideClassName("", sx.bgWhite)}>
				<img {...stylex.props(sx.size10)} src={DAYTONA_FAVICON} alt="" aria-hidden="true" />
			</SandboxProviderTile>
		);
	}
	...
	if (provider === "microvm") {
		return (
			<SandboxProviderTile className={mergeStylexOverrideClassName("", sx.bg111318)}>
				<img {...stylex.props(sx.size6)} src={FIRECRACKER_FAVICON} alt="" aria-hidden="true" />
			</SandboxProviderTile>
		);
	}
	...
	if (provider === "docker") {
		return (
			<SandboxProviderTile className={mergeStylexOverrideClassName("", sx.bgEdf7ff, sx.text2496ed)}>
				<svg viewBox="0 0 24 24" {...stylex.props(sx.size6)} fill="currentColor" aria-hidden="true">
	...
	return (
		<SandboxProviderTile className={mergeStylexOverrideClassName("", sx.bg07140d, sx.text62de61)}>
			<svg viewBox="0 0 24 24" {...stylex.props(sx.size6)} fill="currentColor" aria-hidden="true">
