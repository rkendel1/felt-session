...
const HOVERS = argv.filter((a) => a.startsWith("--hover=")).map((a) => a.slice(8));
...
const REMOVES = argv.filter((a) => a.startsWith("--remove=")).map((a) => a.slice(9));
...
await send("Emulation.setDeviceMetricsOverride", {
	width: VW, height: VH, deviceScaleFactor: 1, mobile: false, screenWidth: VW, screenHeight: VH,
});
...
for (const c of CLICKS) {
	const hit = await evaluate(`(() => { const el = document.querySelector(${JSON.stringify(c)});
...
for (const a of ADDS) {
	...
	const r = await evaluate(`(() => { const el = document.querySelector(${JSON.stringify(sel)});
...
for (const rm of REMOVES) {
	const n = await evaluate(`(() => { const els = [...document.querySelectorAll(${JSON.stringify(rm)})];
...
for (const h of HOVERS) {
	...
	const { root } = await send("DOM.getDocument", { depth: 1 }).then((r: any) => r.result);
	const { nodeIds } = await send("DOM.querySelectorAll", { nodeId: root.nodeId, selector: h }).then((r: any) => r.result);
...
const snapshot = async (): Promise<any[]> => {
	...
	const CH = 4000;
	const out: any[] = [];
	for (let i = 0; i < n; i += CH) out.push(...(await evaluate(`window.__rkSnap.slice(${i}, ${i + CH})`)));
	return out;
};
...
const diffSnaps = (a: any[], b: any[]) => {
	...
	for (const [p, ea] of ma) {
		const eb = mb.get(p);
		if (!eb) { changed.push(`GONE ${p}`); continue; }
		if (ea[1] !== eb[1]) changed.push(`STYLE ${p} ${ea[6]?.slice(0, 70)}`);
		else if (ea[2] !== eb[2] || ea[3] !== eb[3] || ea[4] !== eb[4] || ea[5] !== eb[5])
			changed.push(`RECT ${p} ${ea[2]},${ea[3]} ${ea[4]}x${ea[5]} => ${eb[2]},${eb[3]} ${eb[4]}x${eb[5]}  ${ea[6]?.slice(0, 60)}`);
	}
...
const unmatched = Object.entries(matches).filter(([, n]) => n === 0).map(([s]) => s);
console.log("\nmatched on this page:");
for (const [s, n] of Object.entries(matches)) console.log(`   ${String(n).padStart(4)}  ${s}`);
if (unmatched.length) console.log(`   ^ ${unmatched.length} selector(s) match NOTHING here — their verdict below is vacuous, measure them on a page that renders them`);
...
console.log(`noise floor (same page, nothing deleted): ${floor.length} ${floor.length ? "(excluded from every verdict below)" : "OK"}`);
...
const run = async (sels: string[], label: string) => {
	...
	const churn = new Set(diffSnaps(base, await snapshot()).map((c) => c.split(" ")[1]));
	const real = d.filter((c) => !churn.has(c.split(" ")[1]));
	if (churn.size) console.log(`   (page churn during the run, excluded: ${churn.size} element(s))`);
	...
	const verdict = r.killed === 0 ? "NO RULE MATCHED — nothing was measured" : real.length ? "LIVE" : "DEAD";
	console.log(`${label}: killed ${r.killed} rule(s) => ${real.length} change(s) ${verdict}`);
