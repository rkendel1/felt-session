...
function canAccessMemoryScope(ctx: RouteContext, scopeKey: string): boolean {
	...
	if (scopeKey.startsWith("user-")) {
		...
		return team.some(
			(member) =>
				member.github?.trim().toLowerCase() === login && member.slackId === slackId,
		);
	}
...
function validState(value: unknown): value is MemoryState {
	return typeof value === "string" && MEMORY_STATES.includes(value as MemoryState);
}
...
function requireStatusExpiry(
	...
	if (kind === "status" && (typeof expiresAt !== "string" || !expiresAt.trim())) {
...
export async function handleMemoryRoutes(
	...
	try {
		...
		if (path === "/api/memory/scopes" && req.method === "GET") {
			...
			const visibleStats = stats.scopes.filter((scope) => canAccessMemoryScope(ctx, scope.scopeKey));
			...
			const byKey = new Map(visibleStats.map((scope) => [scope.scopeKey, scope]));
			const keys = new Set([
				"workspace",
			...
			const scopes = [...keys]
				...
				.filter((item): item is typeof item & { scope: NonNullable<typeof item.scope> } => !!item.scope)
			...
			return Response.json({
				...
				stats: {
					...
					reviewCount: visibleStats.reduce((sum, scope) => sum + scope.review, 0),
				},
			});
		}
		...
		if (path === "/api/memory" && req.method === "GET") {
			...
			const visibleKeys = store.stats().scopes
				.filter((scope) => canAccessMemoryScope(ctx, scope.scopeKey))
			...
			const filters: MemoryFilters = {
				...
				confirmed:
					review === "needs_review" ? false : review === "confirmed" ? true : undefined,
			};
			const page = {
...
function legacySummaryEntry(scope: MemoryScope, entry: {
	id: string;
	text: string;
	by: string;
	at: string;
	...
	return {
		id: entry.id,
		scopeKey: scope.key,
		summary: entry.text.length > 400 ? `${entry.text.slice(0, 399)}…` : entry.text,
...
async function handleLegacyMemoryRoutes(ctx: RouteContext): Promise<Response | undefined> {
	const { req, url, path } = ctx;
	const all = async () => (await listAllMemory(Object.keys(REPOS)))
		.filter((item) => canAccessMemoryScope(ctx, item.scope.key));
	if (path === "/api/memory/scopes" && req.method === "GET") {
		...
		return Response.json({
			scopes: scoped.map(({ scope, entries }) => ({
				...
				ambientChars: entries.reduce((sum, entry) => sum + entry.text.length, 0),
			})),
			stats: {
				mode: "legacy",
				...
				ambientUsedBytes: scoped.reduce(
					(sum, item) => sum + item.entries
						...
						.reduce((entrySum, entry) => entrySum + Buffer.byteLength(entry.text, "utf8"), 0),
					0,
				),
	...
	if (path === "/api/memory" && req.method === "GET") {
		...
		const scoped = (await all()).filter((item) => !scopeKey || item.scope.key === scopeKey);
		const query = url.searchParams.get("q")?.toLowerCase();
		const state = url.searchParams.get("state") || "active";
		const kind = url.searchParams.get("kind");
		const review = url.searchParams.get("review");
		if ((kind && kind !== "reference") || state === "expired" || state === "superseded" || review === "confirmed") {
			return Response.json({ items: [] });
		}
		const items = scoped.flatMap(({ scope, entries }) => entries
			.filter((entry) => state === "archived" ? !!entry.archivedAt : !entry.archivedAt)
	...
	const scopeKey = url.searchParams.get("scopeKey") ||
		(req.method === "PATCH" ? String((await req.clone().json().catch(() => null))?.scopeKey || "") : "");
	...
	if (req.method === "GET") {
		return Response.json({ entry: { ...legacySummaryEntry(scope, current), details: current.text } });
	}
	if (req.method === "PATCH") {
		const body = await req.json().catch(() => null);
		...
		invalidateMemorySnapshot();
		const updated = (await all()).find((item) => item.scope.key === scope.key)?.entries.find((entry) => entry.id === id) || current;
		return Response.json({ entry: legacySummaryEntry(scope, updated) });
	}
	if (req.method === "DELETE") {
		if (url.searchParams.get("confirm") !== "true") {
			return Response.json({ error: "confirm=true is required for permanent deletion" }, { status: 400 });
		}
		if (!current.archivedAt) {
			return Response.json({ error: "Archive this memory before deleting it permanently" }, { status: 400 });
		}
